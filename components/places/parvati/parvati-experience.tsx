...
import { PARVATI_FEATURES, type ParvatiFeature } from "@/lib/data/parvati-hill-temple";
import type { FeatureId, HillMode, TimeOfDay } from "@/components/places/parvati/parvati-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const ParvatiScene = dynamic(() => import("@/components/places/parvati/parvati-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Sunrise climb", icon: Sunrise },
  { id: "golden", label: "Golden hour", icon: Sun },
  { id: "dusk", label: "City lights", icon: Moon },
];

const MODE_OPTIONS: { id: HillMode; label: string; icon: typeof Sun }[] = [
  { id: "darshan", label: "Darshan day", icon: Flame },
  { id: "mahashivratri", label: "Mahashivratri", icon: Sparkles },
];
...
export function ParvatiExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const [mode, setMode] = React.useState<HillMode>("darshan");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: ParvatiFeature | undefined = React.useMemo(
    () => PARVATI_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Climbing the 103 steps…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Parvati Hill, showing the 103 stone steps
        climbing from Parvati Paytha, the hill gateway, the Devdeveshwar temple and its shikhara,
        the smaller shrines, the Peshwa museum, and the city panorama from the summit parapet.
        Use the buttons above to change the light, the mode and the viewpoint.
      </p>
    </section>
  );
}
...
function ParvatiFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 55% at 50% 18%, #8a9a5b 0%, transparent 55%), linear-gradient(160deg, #4c5a3a, #2b2a1e 55%, #16130c)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of Parvati
          Hill — the 103 steps, the gateway, the Devdeveshwar temple, the museum and the view.
        </p>
      </div>
    </div>
  );
