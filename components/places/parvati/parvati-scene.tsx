...
export type ParvatiSceneProps = {
  ...
  onReady?: () => void;
  onUnsupported?: () => void;
  /** Bump to re-frame the camera on the whole hill. */
  resetSignal?: number;
};

/** React ↔ three.js bridge; the world is created once and driven by props. */
export default function ParvatiScene({
  active,
  timeOfDay,
  mode,
  onSelect,
  ...
  const worldRef = React.useRef<HillWorld | null>(null);
  ...
  React.useEffect(() => {
    ...
    let world: HillWorld;
    try {
      world = createParvatiWorld(mount, {
        reducedMotion,
