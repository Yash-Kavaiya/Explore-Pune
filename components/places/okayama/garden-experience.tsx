...
import { GARDEN_FEATURES, type GardenFeature } from "@/lib/data/okayama-garden";
import type { FeatureId, Season, TimeOfDay } from "@/components/places/okayama/garden-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const GardenScene = dynamic(() => import("@/components/places/okayama/garden-scene"), {
  ssr: false,
});
...
export function GardenExperience({
  ...
  const activeFeature: GardenFeature | undefined = React.useMemo(
    () => GARDEN_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        {/* Scrims keep the overlay text legible whatever the sky is doing. */}
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Planting the garden…</p>
          </div>
        </div>
          ...
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-xl">
              <div className="flex flex-wrap items-center gap-2">
                <CategoryBadge category="nature-gardens" className="border-white/25 bg-white/15 text-white backdrop-blur-md" />
      ...
      {/* The scene is decorative; this is its accessible equivalent. */}
      <p className="sr-only">
        An interactive three-dimensional model of the Pune-Okayama Friendship Garden, showing the
        cascade, the arched bridge, the lantern walk, the pavilion, the pond with its islands and the
        bamboo grove. Use the buttons above to change the light, the season and the viewpoint.
      </p>
    </section>
  );
}
...
function FeatureCard({
  ...
  return (
    <div className="relative rounded-2xl border border-white/15 bg-black/50 p-4 text-white backdrop-blur-xl">
      ...
      <p className="mt-0.5 text-xs uppercase tracking-wider text-white/50">{feature.japanese}</p>
      <p className="mt-2 text-sm leading-relaxed text-white/85">{feature.blurb}</p>
    </div>
  );
}

/** Shown when the browser can't give us WebGL at all. */
function GardenFallback() {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 60% at 30% 20%, #6f9a4e 0%, transparent 60%), linear-gradient(150deg, #2f5f57, #1d3b3f 55%, #101f26)",
      }}
    >
      <div className="absolute inset-0 opacity-25 mix-blend-soft-light [background-image:radial-gradient(circle_at_1px_1px,rgba(255,255,255,0.6)_1px,transparent_0)] [background-size:16px_16px]" />
    </div>
  );
}
