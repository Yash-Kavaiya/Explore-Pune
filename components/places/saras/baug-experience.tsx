...
import { BAUG_FEATURES, type BaugFeature } from "@/lib/data/saras-baug";
import type { BaugMode, FeatureId, TimeOfDay } from "@/components/places/saras/baug-world";

const BaugScene = dynamic(() => import("@/components/places/saras/baug-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning lawn", icon: Sunrise },
  { id: "golden", label: "Late afternoon", icon: Sun },
  { id: "dusk", label: "Family evening", icon: Moon },
];

const MODE_OPTIONS: { id: BaugMode; label: string; icon: typeof Sun }[] = [
  { id: "day", label: "Quiet day", icon: Sun },
  { id: "evening", label: "Aarti evening", icon: Flame },
];
...
export function BaugExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const [mode, setMode] = React.useState<BaugMode>("evening");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: BaugFeature | undefined = React.useMemo(
    () => BAUG_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Crossing the old lake bed…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Saras Baug, showing Talyatla Ganpati on the
        former lake-island mound, the drained tank now lawns and a walking path, the small
        Ganesh museum, evening snack stalls, and Parvati Hill next door. Use the buttons above
        to change the light, the mode and the viewpoint.
      </p>
    </section>
  );
}
...
function BaugFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 55% 45% at 50% 60%, #5a7a40 0%, transparent 55%), linear-gradient(170deg, #2a3220, #141810 55%, #0a0c08)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          ...
          Saras Baug — Talyatla Ganpati, the drained lake, and Parvati next door.
        </p>
      </div>
    </div>
  );
