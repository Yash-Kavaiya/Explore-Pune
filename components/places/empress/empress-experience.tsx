...
import { EMPRESS_FEATURES, type EmpressFeature } from "@/lib/data/empress-garden";
import type { EmpressMode, FeatureId, TimeOfDay } from "@/components/places/empress/empress-world";

const EmpressScene = dynamic(() => import("@/components/places/empress/empress-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning canopy", icon: Sunrise },
  { id: "golden", label: "Afternoon lawn", icon: Sun },
  { id: "dusk", label: "Show evening", icon: Moon },
];

const MODE_OPTIONS: { id: EmpressMode; label: string; icon: typeof Sun }[] = [
  { id: "ordinary", label: "Ordinary day", icon: Sun },
  { id: "show", label: "Flower show", icon: Sparkles },
];
...
export function EmpressExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const [mode, setMode] = React.useState<EmpressMode>("show");
  ...
  const activeFeature = React.useMemo(
    () => EMPRESS_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Walking under the canopy…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Empress Garden, showing the old-growth canopy,
        rolling lawns, the formal rose garden, the greenhouse, and the annual flower-show tents.
...
function EmpressFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 55% 45% at 50% 60%, #4a7a38 0%, transparent 55%), linear-gradient(170deg, #243020, #121610 55%, #0a0c08)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          ...
          Empress Garden — the canopy, the rose beds, the greenhouse and the January show.
        </p>
      </div>
    </div>
  );
