...
import { MEMORIAL_FEATURES, type MemorialFeature } from "@/lib/data/national-war-memorial";
import type {
  FeatureId,
  MemorialMode,
  TimeOfDay,
...
const MemorialScene = dynamic(() => import("@/components/places/memorial/memorial-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning parade", icon: Sunrise },
  { id: "golden", label: "Daylight grounds", icon: Sun },
  { id: "dusk", label: "Show lighting", icon: Moon },
];

const MODE_OPTIONS: { id: MemorialMode; label: string; icon: typeof Sun }[] = [
  { id: "daylight", label: "Open grounds", icon: Flame },
  { id: "ceremony", label: "Sound & light", icon: Sparkles },
];
...
export function MemorialExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const [mode, setMode] = React.useState<MemorialMode>("daylight");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: MemorialFeature | undefined = React.useMemo(
    () => MEMORIAL_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Approaching the column…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of the National War Memorial Southern Command,
        showing the 25-metre memorial column, the eternal flame, outdoor tanks and artillery,
        a MiG-23BN fighter, the INS Trishul replica, and the Southern Command Museum in the
        gardens. Use the buttons above to change the light, the mode and the viewpoint.
      </p>
    </section>
  );
}
...
function MemorialFallback() {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "radial-gradient(ellipse 50% 45% at 50% 58%, #6a5a42 0%, transparent 55%), linear-gradient(170deg, #2a241c, #14110c 55%, #0a0908)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          ...
          the National War Memorial — the column, the eternal flame, the MiG-23BN and INS Trishul.
        </p>
      </div>
    </div>
  );
