...
import { TEMPLE_FEATURES, type TempleFeature } from "@/lib/data/dagdusheth-temple";
import type { FeatureId, TempleMode, TimeOfDay } from "@/components/places/temple/temple-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const TempleScene = dynamic(() => import("@/components/places/temple/temple-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Mangal aarti", icon: Sunrise },
  { id: "golden", label: "Evening aarti", icon: Sun },
  { id: "dusk", label: "Utsav night", icon: Moon },
];

const MODE_OPTIONS: { id: TempleMode; label: string; icon: typeof Sun }[] = [
  { id: "darshan", label: "Darshan day", icon: Flame },
  { id: "utsav", label: "Ganeshotsav", icon: Sparkles },
];
...
export function TempleExperience({
  ...
  const [mode, setMode] = React.useState<TempleMode>("darshan");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: TempleFeature | undefined = React.useMemo(
    () => TEMPLE_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Lighting the deepams…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of the Dagdusheth Halwai Ganapati Temple, showing
        the mahadwar gateway, the gold-adorned idol in the sanctum, the sabha mandap, the twin
        deepmalas, the courtyard and the Ganeshotsav pandal. Use the buttons above to change the
        light, the mode and the viewpoint.
      </p>
    </section>
  );
}
...
function TempleFallback() {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 60% at 35% 25%, #d99a3c 0%, transparent 55%), linear-gradient(150deg, #6b3a1e, #2a1610 55%, #140c08)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of Dagdusheth
          Halwai Ganapati — the mahadwar, gold-adorned idol, sabha mandap, deepmalas and Ganeshotsav.
        </p>
      </div>
    </div>
  );
