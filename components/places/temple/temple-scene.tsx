...
export type TempleSceneProps = {
  ...
  onReady?: () => void;
  onUnsupported?: () => void;
  /** Bump to re-frame the camera on the whole temple. */
  resetSignal?: number;
};
...
export default function TempleScene({
  ...
  const worldRef = React.useRef<TempleWorld | null>(null);
  ...
  React.useEffect(() => {
    ...
    let world: TempleWorld;
    try {
      world = createTempleWorld(mount, {
        reducedMotion,
