...
import { CAVE_FEATURES, type CaveFeature } from "@/lib/data/pataleshwar-cave";
import type { CaveMode, FeatureId, TimeOfDay } from "@/components/places/pataleshwar/cave-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const CaveScene = dynamic(() => import("@/components/places/pataleshwar/cave-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning court", icon: Sunrise },
  { id: "golden", label: "Afternoon rock", icon: Sun },
  { id: "dusk", label: "Lamp-lit cave", icon: Moon },
];

const MODE_OPTIONS: { id: CaveMode; label: string; icon: typeof Sun }[] = [
...
export function CaveExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const [mode, setMode] = React.useState<CaveMode>("darshan");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: CaveFeature | undefined = React.useMemo(
    () => CAVE_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Descending into the court…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Pataleshwar Cave Temple, showing the sunken
        monolithic court below Jangli Maharaj Road, the circular Nandi mandapa, the cave mouth,
        the pillared rock-cut hall, the lower linga sanctum, and the unfinished 8th-century rock.
...
function CaveFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 60% 50% at 50% 62%, #5a5348 0%, transparent 55%), linear-gradient(170deg, #2a2620, #16130e 55%, #0c0a08)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          ...
          Pataleshwar — the sunken court, the circular Nandi, the rock-cut hall and the linga.
        </p>
      </div>
    </div>
  );
