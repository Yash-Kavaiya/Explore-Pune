...
import { DAM_FEATURES, type DamFeature } from "@/lib/data/khadakwasla-dam";
import type { DamMode, FeatureId, TimeOfDay } from "@/components/places/dam/dam-world";

const DamScene = dynamic(() => import("@/components/places/dam/dam-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning lake", icon: Sunrise },
  { id: "golden", label: "Sunset water", icon: Sun },
  { id: "dusk", label: "Chowpatty night", icon: Moon },
];

const MODE_OPTIONS: { id: DamMode; label: string; icon: typeof Sun }[] = [
  { id: "dry", label: "Dry season", icon: Sun },
  { id: "monsoon", label: "Monsoon full", icon: CloudRain },
];
...
export function DamExperience({
  ...
  const [mode, setMode] = React.useState<DamMode>("monsoon");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: DamFeature | undefined = React.useMemo(
    () => DAM_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Walking out onto the wall…</p>
          </div>
        </div>
        ...
        <div className="absolute inset-x-0 top-0 flex flex-wrap items-start justify-between gap-3 p-4 sm:p-6">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <Segmented label="Season" value={mode} options={MODE_OPTIONS} onChange={setMode} />
            <button
              type="button"
              onClick={() => {
                setActive(null);
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Khadakwasla Dam, showing the long masonry
        wall across the Mutha, Khadakwasla Lake, eleven radial sluice gates, the chowpatty
        promenade of evening vendors, and Sinhagad Fort on the ridge behind the water. Use
        the buttons above to change the light, the season and the viewpoint.
      </p>
    </section>
  );
}
...
function DamFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 55% 45% at 50% 62%, #3a6a68 0%, transparent 55%), linear-gradient(170deg, #243848, #141820 55%, #0a0c10)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          ...
          Khadakwasla — the Mutha wall, the lake, the sluices and Sinhagad on the ridge.
        </p>
      </div>
    </div>
  );
