...
import { MUSEUM_FEATURES, type MuseumFeature } from "@/lib/data/kelkar-museum";
import type {
  FeatureId,
  MuseumMode,
  TimeOfDay,
...
const MuseumScene = dynamic(() => import("@/components/places/kelkar/museum-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning light", icon: Sunrise },
  { id: "golden", label: "Afternoon", icon: Sun },
  { id: "dusk", label: "Evening", icon: Moon },
];

const MODE_OPTIONS: { id: MuseumMode; label: string; icon: typeof Sun }[] = [
  { id: "open", label: "Open hours", icon: Sun },
  { id: "evening", label: "Gallery lamps", icon: Lamp },
];
...
export function MuseumExperience({
  ...
  const [mode, setMode] = React.useState<MuseumMode>("open");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: MuseumFeature | undefined = React.useMemo(
    () => MUSEUM_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Opening the galleries…</p>
          </div>
        </div>
        ...
        <div className="absolute inset-x-0 top-0 flex flex-wrap items-start justify-between gap-3 p-4 sm:p-6">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            <Segmented label="Galleries" value={mode} options={MODE_OPTIONS} onChange={setMode} />
            <button
              type="button"
              onClick={() => {
                setActive(null);
      ...
      <p className="sr-only">
        An interactive three-dimensional model of the Raja Dinkar Kelkar Museum, showing the
        Mastani Mahal, lamps gallery, musical instruments, betel-nut cutters, carved doors and the
        armoury. Use the buttons above to change the light, the gallery lamps and the viewpoint.
      </p>
    </section>
  );
}
...
function MuseumFallback() {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 55% at 40% 30%, #c9a06a 0%, transparent 55%), linear-gradient(155deg, #5c4030, #2a1c14 55%, #120e0c)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          ...
          Raja Dinkar Kelkar Museum — Mastani Mahal, lamps, instruments and more.
        </p>
      </div>
    </div>
  );
