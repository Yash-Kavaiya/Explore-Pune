...
import {
  ...
  OSHO_STATS,
  OSHO_STORY,
} from "@/lib/data/osho-resort";

/**
 * Editorial half of the Osho Meditation Resort page — the history and
 * features the 3D model can show but not explain. Feature cards match the
 * scene markers.
 */
export function OshoStory() {
  return (
    <>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:gap-16">
          <div className="lg:sticky lg:top-24 lg:h-fit">
            ...
            <h2 className="mt-4 text-balance font-heading text-3xl font-bold tracking-tight sm:text-4xl">
              The strange, serene world on Lane 1
            </h2>
            <figure className="mt-6 border-l-2 border-primary/40 pl-4">
              ...
              <blockquote className="mt-2 text-sm italic leading-relaxed text-muted-foreground">
                The city ends at the gate. Inside, the loudest thing is the bamboo.
              </blockquote>
            </figure>
          </div>

          <div className="space-y-10">
            {OSHO_STORY.map((block) => (
...
export function OshoEtiquette() {
  return (
    <div className="rounded-2xl border border-border/70 bg-card p-6">
      <h3 className="font-heading text-lg font-semibold">Good to know</h3>
      <ul className="mt-4 space-y-3">
        {OSHO_ETIQUETTE.map((rule) => (
