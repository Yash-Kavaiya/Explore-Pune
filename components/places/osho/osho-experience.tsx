...
import { OSHO_FEATURES, type OshoFeature } from "@/lib/data/osho-resort";
import type { FeatureId, ResortMode, TimeOfDay } from "@/components/places/osho/osho-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const OshoScene = dynamic(() => import("@/components/places/osho/osho-scene"), {
  ssr: false,
});

const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  { id: "dawn", label: "Morning sitting", icon: Sunrise },
  { id: "golden", label: "Afternoon", icon: Sun },
  { id: "dusk", label: "Dusk", icon: Moon },
];

const MODE_OPTIONS: { id: ResortMode; label: string; icon: typeof Sun }[] = [
  { id: "day", label: "Quiet day", icon: Leaf },
  { id: "celebration", label: "Evening celebration", icon: Music },
];
...
export function OshoExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const [mode, setMode] = React.useState<ResortMode>("day");
  const [ready, setReady] = React.useState(false);
  const [unsupported, setUnsupported] = React.useState(false);
  const [resetSignal, setResetSignal] = React.useState(0);
  const [touched, setTouched] = React.useState(false);

  const activeFeature: OshoFeature | undefined = React.useMemo(
    () => OSHO_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Entering Koregaon Park…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of the Osho International Meditation Resort,
        showing the welcome gate, the black pyramid auditorium, the Zen garden, the swimming
        pool, the stream and ponds of Osho Teerth park, and the evening celebration lighting.
...
function OshoFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 55% at 50% 20%, #5c7c3e 0%, transparent 55%), linear-gradient(160deg, #31452a, #1d2418 55%, #10130c)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of the
          Osho resort — the pyramid, the Zen garden, the pool and Osho Teerth park.
        </p>
      </div>
    </div>
  );
