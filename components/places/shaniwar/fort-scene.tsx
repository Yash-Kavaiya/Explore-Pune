...
import {
  ...
  type Season,
  type TimeOfDay,
} from "@/components/places/shaniwar/fort-world";

export type FortSceneProps = {
  ...
  onReady?: () => void;
  onUnsupported?: () => void;
  /** Bump to re-frame the camera on the whole fort. */
  resetSignal?: number;
};

/** React ↔ three.js bridge; the world is created once and driven by props. */
export default function FortScene({
  active,
  timeOfDay,
  season,
  onSelect,
  ...
  const worldRef = React.useRef<FortWorld | null>(null);
