...
import { FORT_FEATURES, type FortFeature } from "@/lib/data/shaniwar-wada";
import type { FeatureId, Season, TimeOfDay } from "@/components/places/shaniwar/fort-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const FortScene = dynamic(() => import("@/components/places/shaniwar/fort-scene"), {
  ssr: false,
});
...
const LIGHT_OPTIONS: { id: TimeOfDay; label: string; icon: typeof Sun }[] = [
  ...
  { id: "dusk", label: "Light show", icon: Moon },
];

/** Season tints the sky/ground and toggles rain — works with any light mode. */
const SEASON_OPTIONS: { id: Season; label: string; icon: typeof Sun }[] = [
  { id: "dry", label: "Winter", icon: Snowflake },
...
export function FortExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const activeFeature: FortFeature | undefined = React.useMemo(
    () => FORT_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Raising the ramparts…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Shaniwar Wada, showing the Delhi Darwaza, ramparts
        and bastions with saffron flags on all four corner bastions, the Hazari Karanje fountain,
        palace foundations, the nagarkhana and the lawns. Use the buttons above to change the light
        (dawn, golden hour, light show), the season (winter or monsoon), and the viewpoint.
      </p>
    </section>
  );
}
...
function FortFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 60% at 35% 25%, #c4784a 0%, transparent 55%), linear-gradient(150deg, #5c3a28, #2a1a14 55%, #120c0a)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of Shaniwar
          Wada — Delhi Darwaza, ramparts, Hazari Karanje, and the palace foundations.
        </p>
      </div>
    </div>
  );
