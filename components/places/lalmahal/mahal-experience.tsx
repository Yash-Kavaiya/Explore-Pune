...
import { LAL_MAHAL_FEATURES, type LalMahalFeature } from "@/lib/data/lal-mahal";
import type { FeatureId, Season, TimeOfDay } from "@/components/places/lalmahal/mahal-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const MahalScene = dynamic(() => import("@/components/places/lalmahal/mahal-scene"), {
  ssr: false,
});
...
export function MahalExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const activeFeature: LalMahalFeature | undefined = React.useMemo(
    () => LAL_MAHAL_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Raising the red walls…</p>
          </div>
        </div>
          ...
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="lg:max-w-md lg:shrink-0">
              ...
              <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] lg:flex-wrap lg:justify-end lg:overflow-visible">
                {LAL_MAHAL_FEATURES.map((feature, i) => {
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Lal Mahal, the red palace of Shivaji’s childhood
        in Kasba Peth — the reconstructed facade, Jijabai’s wing, the Shaista Khan hall memory, the
        painting gallery, garden grounds and street entrance. Use the buttons above to change the
        light, the season and the viewpoint.
      </p>
    </section>
  );
}
...
function MahalFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 60% at 40% 30%, #c46848 0%, transparent 55%), linear-gradient(150deg, #7a3a28, #2a1810 55%, #120c08)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of Lal Mahal
          — Shivaji’s childhood home, Jijabai, the Shaista Khan night, and the red-palace
          reconstruction in Kasba Peth.
        </p>
      </div>
    </div>
  );
