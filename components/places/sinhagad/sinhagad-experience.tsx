...
import { SINHAGAD_FEATURES, type SinhagadFeature } from "@/lib/data/sinhagad-fort";
import type { FeatureId, Season, TimeOfDay } from "@/components/places/sinhagad/sinhagad-world";

const SinhagadScene = dynamic(() => import("@/components/places/sinhagad/sinhagad-scene"), {
  ssr: false,
});
...
export function SinhagadExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const activeFeature: SinhagadFeature | undefined = React.useMemo(
    () => SINHAGAD_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Raising the ridge…</p>
          </div>
        </div>
          ...
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="lg:max-w-md lg:shrink-0">
              ...
              <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] lg:flex-wrap lg:justify-end lg:overflow-visible">
                {SINHAGAD_FEATURES.map((feature, i) => {
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Sinhagad Fort, showing Kalyan Darwaja, Tanaji&apos;s
        memorial, the trek trail, the panoramic lookout, hilltop stalls and Pune Darwaja. Use the
        buttons above to change the light, the season and the viewpoint.
      </p>
    </section>
  );
}
...
function SinhagadFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 55% at 40% 28%, #c4784a 0%, transparent 55%), linear-gradient(155deg, #5c4030, #2a1c14 55%, #120e0c)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of Sinhagad
          Fort — Tanaji&apos;s memorial, the trek trail, gates and hilltop stalls.
        </p>
      </div>
    </div>
  );
