...
import { PALACE_FEATURES, type PalaceFeature } from "@/lib/data/aga-khan-palace";
import type { FeatureId, Season, TimeOfDay } from "@/components/places/agakhan/palace-world";

/** three.js is client-only and heavy — keep it out of the server render. */
const PalaceScene = dynamic(() => import("@/components/places/agakhan/palace-scene"), {
  ssr: false,
});
...
export function PalaceExperience({
  name,
  area,
  rating,
  reviewCount,
  ...
  const activeFeature: PalaceFeature | undefined = React.useMemo(
    () => PALACE_FEATURES.find((f) => f.id === active),
    [active],
  );
  ...
  return (
    <section className="relative isolate overflow-hidden bg-neutral-900">
      <div className="relative h-[80svh] max-h-[880px] min-h-[560px] w-full">
        ...
        >
          <div className="flex flex-col items-center gap-3 text-white/80">
            ...
            <p className="font-heading text-sm tracking-wide">Raising the arches…</p>
          </div>
        </div>
      ...
      <p className="sr-only">
        An interactive three-dimensional model of Aga Khan Palace, showing the Italianate facade,
        the arched arcade, the Gandhi room, the samadhis of Kasturba Gandhi and Mahadev Desai, the
        lawns and the memorial museum. Use the buttons above to change the light, the season and the
        viewpoint.
      </p>
    </section>
  );
...
function PalaceFallback() {
  return (
    <div
      className="absolute inset-0"
      style={{
        backgroundImage:
          "radial-gradient(ellipse 70% 60% at 35% 25%, #d8c49a 0%, transparent 55%), linear-gradient(150deg, #7a6244, #33261a 55%, #171009)",
      }}
    >
      ...
      <div className="absolute inset-0 flex items-end p-8">
        <p className="max-w-md text-sm text-white/70">
          Interactive 3D is unavailable in this browser. Scroll down for the full story of Aga Khan
          Palace — the Italianate facade, Gandhi room, samadhis and memorial museum.
        </p>
      </div>
    </div>
  );
